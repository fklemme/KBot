...
namespace KBot {
    ...
    public:
        ...
        // The allocation of resources for tasks with the same priority is "greedy". If a task is able to allocate all its resources, it will do so.
        // For tasks with different priorities, the lesser tasks will always have to wait for the higher tasks to allocate their resources.
    ...
    public:
        BuildTask(Manager &manager, BWAPI::UnitType toBuild, Priority priority = Priority::normal,
            BWAPI::TilePosition position = BWAPI::Broodwar->self()->getStartLocation(), bool exactPosition = false);
